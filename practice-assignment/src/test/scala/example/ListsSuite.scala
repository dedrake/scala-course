...
class ListsSuite extends FunSuite {
  ...
  test("sum of all zeroes") {
    assert(sum(List(0,0,0)) === 0)
  }
  test("sum of empty List object") {
    assert(sum(List()) === 0)
  }
  ...
  test("max is first element") {
    assert(max(List(13, 7, 2)) === 13)
  }
  test("max of a few numbers including negatives") {
    assert(max(List(-3, -7, -2)) === -2)
  }
  test("max of all zeroes") {
    assert(max(List(0, 0, 0)) === 0)
  }
  test("max of empty List object") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  test("max of List object with only one element") {
    assert(max(List(5)) === 5)
  }
}
